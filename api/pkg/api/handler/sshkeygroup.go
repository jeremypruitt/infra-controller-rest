...
func (cskgh CreateSSHKeyGroupHandler) Handle(c echo.Context) error {
	...
	err = cdb.WithTx(ctx, cskgh.dbSession, func(tx *cdb.Tx) error {
		...
		if derr != nil {
			logger.Error().Err(derr).Msg("db error retrieving TenantSite records for Tenant")
			return cutil.NewAPIError(http.StatusInternalServerError, "Failed to retrieve Site associations for Tenant, DB error", nil)
		}
		...
		for _, stID := range apiRequest.SiteIDs {
			...
			if serr != nil {
				if serr == common.ErrInvalidID {
					return cutil.NewAPIError(http.StatusBadRequest, fmt.Sprintf("Failed to create SSH Key Group, Invalid Site ID: %s", stID), nil)
				}
				if serr == cdb.ErrDoesNotExist {
					return cutil.NewAPIError(http.StatusNotFound, fmt.Sprintf("Failed to create SSH Key Group, Could not find Site with ID: %s ", stID), nil)
				}
				logger.Warn().Err(serr).Str("Site ID", stID).Msg("error retrieving Site from DB by ID")
				return cutil.NewAPIError(http.StatusBadRequest, fmt.Sprintf("Failed to create SSH Key Group, Could not find Site with ID: %s, DB error", stID), nil)
			}
			...
			if site.Status != cdbm.SiteStatusRegistered {
				...
				return cutil.NewAPIError(http.StatusBadRequest, fmt.Sprintf("Failed to create SSH Key Group, Site: %s specified in request is not in Registered state", site.ID.String()), nil)
			}
		...
		for _, skID := range apiRequest.SSHKeyIDs {
			...
			if serr != nil {
				if serr == common.ErrInvalidID {
					return cutil.NewAPIError(http.StatusBadRequest, fmt.Sprintf("Failed to create SSH Key Group, Invalid SSH Key ID: %s", skID), nil)
				}
				if serr == cdb.ErrDoesNotExist {
					return cutil.NewAPIError(http.StatusBadRequest, fmt.Sprintf("Failed to create SSH Key Group, Could not find SSH Key with ID: %s ", skID), nil)
				}

				logger.Warn().Err(serr).Str("SSH Key ID", skID).Msg("error retrieving SSH Key from DB by ID")
				return cutil.NewAPIError(http.StatusBadRequest, fmt.Sprintf("Failed to create SSH Key Group, Could not find SSH Key with ID: %s, DB error", skID), nil)
			}
			...
			if sshkey.TenantID != tenant.ID {
				...
				return cutil.NewAPIError(http.StatusBadRequest, fmt.Sprintf("Failed to create SSH Key Group, SSH Key with ID: %s does not belong to Tenant", skID), nil)
			}
