...
func (dith DeleteInstanceTypeHandler) Handle(c echo.Context) error {
	...
	err = cdb.WithTx(ctx, dith.dbSession, func(tx *cdb.Tx) error {
		...
		// request can attach this Instance Type between the pre-flight usage
		// checks above and DeleteByID below — a TOCTOU window that would
		// either turn into a generic 500 from a DB constraint or, worse,
		// delete a now-in-use record.
		derr := tx.TryAcquireAdvisoryLock(ctx, cdb.GetAdvisoryLockIDFromString(it.ID.String()), nil)
		if derr != nil {
			logger.Error().Err(derr).Msg("failed to acquire advisory lock on Instance Type")
		...
		// Re-run the "is in use" checks inside the locked tx so a concurrent
		// attach can't slip in between the pre-flight checks and the delete.
		instances, _, derr := iDAO.GetAll(ctx, tx, cdbm.InstanceFilterInput{InstanceTypeIDs: []uuid.UUID{it.ID}}, cdbp.PageInput{}, nil)
		if derr != nil {
			logger.Error().Err(derr).Msg("error rechecking Instances for Instance Type from DB")
		...
		if derr != nil {
			logger.Error().Err(derr).Msg("error rechecking Allocation Constraints for Instance Type from DB")
