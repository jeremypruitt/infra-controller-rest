...
func (umh DeleteMachineHandler) Handle(c echo.Context) error {
	...
	err = cdb.WithTx(ctx, umh.dbSession, func(tx *cdb.Tx) error {
		...
		if lastStatus.Status != cdbm.MachineStatusMissing {
			logger.Error().Str("Latest Status", lastStatus.Status).Msg("IsMissingOnSite is true but most recent status is not Missing")
